var dsidx = (function () {
	var returnObj;
	var activatedActions = {};
	var activatedActionCallbacks = {};
	var $ = jQuery;

	function InitializeLayout() {
		var post_body = $('.post-' + dsidx.contentDomId);
		var article_header = $('article header');
		var article = $('article');

		if (post_body.length == 1 || article_header.length == 1 || article.length == 1) {
			var search = $('.dsidx-results #dsidx-top-search').detach();
			var profile = $('#dsidx-profile-header').detach();
			var actions = $('.dsidx-top-actions').detach();

			if (!profile.length && !actions.length)
				return;

			var dsidx_header = $(document.createElement('div'))
			.addClass('dsidx-content-header')
			.append(search)
			.append(profile)
			.append(actions)
			.append('<div class="dsidx-clear"></div>');

			if (post_body.length == 1)
				post_body.prepend(dsidx_header);
			else if (article_header.length == 1)
				article_header.before(dsidx_header);
			else if (article.length == 1)
				article.before(dsidx_header);
		}
	}
	...
	returnObj = {
		...
		getVersionId: function () {
			var version = dsidx.pluginVersion;
			var versionId;
			var beta;
			...
			versionId = version.split('.');
			versionId = parseInt(versionId[0]) * 10000 + parseInt(versionId[1]) * 100 + parseInt(versionId[2] ? versionId[2] : 0);
			beta = /\-beta(\d+)$/.exec(version);

			if (beta && beta[1])
				versionId += parseInt(beta[1]) / 100;

			arguments.callee = function () { return versionId; };
			return arguments.callee();
		},
		activate: function (action, opt_callback) {
			activatedActions[action] = true;
			if (opt_callback) {
				activatedActionCallbacks[action] = activatedActionCallbacks[action] || [];
				activatedActionCallbacks[action].push(opt_callback);
			}
		},
		isActivated: function (action) {
			return activatedActions[action];
		},
		ExtractValueFromApiData: function (key, data) {
			var regex = new RegExp('^\\<!\\-\\-\\s*' + key + ':\\s*"([^"]+)"\\s*\\-\\-\\>', 'm');
		...
		trackEvent: function (category, event, label) {
		    if (typeof (zTracking) != 'undefined') {
		        zTracking.trackEvent(category, event, label);
		        return;
		    }
		    if (typeof(_gaq) != 'undefined') { //ga.js (old)
		        _gaq.push(['_trackEvent', category, event, label]);
		    }
		    if (typeof(ga) != 'undefined') { //analytics.js (newer)
		        ga('send', 'event', category, event, label);
		    }
		},
		trackPageview: function (url) {
		    if (typeof (zTracking) != 'undefined') {
		        zTracking.trackPageview(url);
		        return;
		    }
		    if (typeof(_gaq) != 'undefined') {  //ga.js (old)
	...
	$(function () {
		if (typeof isDsIDXPage !== 'undefined') {
			$(document.body).addClass('dsidx');
		}
	    for (var key in activatedActions) {
	        if (activatedActions.hasOwnProperty(key) && typeof dsidx[key] == 'function')
	            dsidx[key] = dsidx[key]();
	        if (activatedActionCallbacks.hasOwnProperty(key)) {
	            for (var i = activatedActionCallbacks[key].length; i--; )
	                activatedActionCallbacks[key][i]();
	        }
	    }
...
/* THIS IS VERY SLIGHTLY CUSTOMIZED FOR OUR (DIVERSE SOLUTIONS) USE SO THAT IT DOES NOT INTERFERE WITH ANY OTHER
 * VERSION OF SWFOBJECT THAT MAY BE LOADED INTO THE PAGE. DO NOT UPGRADE WITHOUT RE-APPLYING dsidx NAMESPACE!
 */
	...
	var cleanup = function() {
		if (ua.ie && ua.win) {
			window.attachEvent("onunload", function() {
				...
				ua = null;
				for (var l in dsidx.swfobject) {
					dsidx.swfobject[l] = null;
				}
...
/* THIS IS VERY SLIGHTLY CUSTOMIZED FOR OUR (DIVERSE SOLUTIONS) USE SO THAT IT DOES NOT INTERFERE WITH ANY OTHER
 * VERSION OF COLORBOX THAT MAY BE LOADED INTO THE PAGE. DO NOT UPGRADE WITHOUT RE-APPLYING dsidx NAMESPACE!
 */
...
dsidx.search = function () {
	...
	if (location.hash == '#expanded-search') {
		$('#dsidx-top-search').addClass('open');
		var $fe = $('#dsidx-top-search #dsidx-search-filters');
		$fe.addClass('open');
		$fe.find('.dsidx-search-filters-heading span.dsidx-search-openclose').html('-');
	}

	$('#dsidx-top-search').click(function () {
		$(this).addClass('open');
	});

	$('#dsidx-top-search .dsidx-search-bar-pull-up').click(function (e) {
		e.stopPropagation();
		$('#dsidx-top-search').removeClass('open');
		$('#dsidx-search-location-display').val($('#dsidx-search-location').val());
	});

	$('#dsidx-top-search #dsidx-search-bar input').click(function (e) {
		e.stopPropagation();
		var $p = $(this).parents('#dsidx-top-search'), $e = $('#dsidx-top-search #' + this.id.replace('-display', ''));
		$p.addClass('open');
		if ($e.length > 0) {
			$e.select();
		}
	...
	$('#dsidx-top-search #dsidx-search-filters .dsidx-search-filters-heading').click(function () {
		var $e = $(this).parent('#dsidx-search-filters');
		if ($e.hasClass('open')) {
			$e.removeClass('open');
			$(this).find('span.dsidx-search-openclose').html('+');
		} else {
			$e.addClass('open');
			$(this).find('span.dsidx-search-openclose').html('-');
		}
	...
	$('#dsidx-top-search .dsidx-search-tag-list').click(function () {
		if (!$(this).hasClass('open')) {
			$(this).addClass('open');
			$(this).find('span.dsidx-search-openclose').html('-');
			$(this).find('p.dsidx-search-empty-list').addClass('hidden');
		}
	});

	$('#dsidx-top-search .dsidx-search-tag-list .dsidx-search-openclose').click(function (e) {
		var $p = $(this).parents('.dsidx-search-tag-list');
		if ($p.hasClass('open')) {
			$p.removeClass('open');
			$(this).html('+');
			if ($p.find(':checked').length == 0)
				$p.find('p.dsidx-search-empty-list').removeClass('hidden');
		} else {
			$p.addClass('open');
			$(this).html('-');
			$p.find('p.dsidx-search-empty-list').addClass('hidden');
		}
		e.stopPropagation();
	});

	$('#dsidx-top-search .dsidx-search-tag-list input[type=checkbox]').click(function () {
		var $p = $(this).parents('.dsidx-search-tag-checkboxes'), list_html = '', input_html = '';
		var $c = $p.find('input[type=checkbox]:checked');
		$.each($c, function (i, cb) {
			list_html += '<li>' + cb.title + '</li>';
			input_html += '<input type="hidden" name="idx-q-PropertyFeatures<' + i + '>" value="' + cb.value + '" />';
		});
		$(this).parents('.dsidx-search-tag-list').find('ul').html(list_html);
		$(this).parents('.dsidx-search-tag-list').find('.dsidx-search-tag-hidden-inputs').html(input_html);
	});

	$('#dsidx-top-search input.dsidx-search-status-option').click(function () {
		var statusEnumHash = 0;
		$('#dsidx-top-search input.dsidx-search-status-option:checked').each(function () {
			statusEnumHash += parseInt($(this).val());
		});
		$('#dsidx-top-search input#dsidx-search-status').val(statusEnumHash);
	});

	$('#dsidx-top-search #dsidx-search-form').submit(function () {
	    // strip out states from the location
	    var loc = $('#dsidx-search-location').val(), loc_final = '', stateInSearch = false;
		var loc_pieces = loc.split(',');
		if (loc_pieces.length > 1) {
			for (var i = 0; i < loc_pieces.length; i++) {
				// rebuild location string
			    if (stateFilter.indexOf(loc_pieces[i].trim().toLowerCase()) == -1) {
			        loc_final += loc_pieces[i];
			    }
			    else {
			        stateInSearch = true;
			    }
		...
		if (loc_final == '' || !stateInSearch) {
			loc_final = loc;
		}
		loc_final = $.trim(loc_final);
		$('#dsidx-search-location').val(loc_final);
		// detect zip code
		if (loc_final.match(/^\d{5}$/) && $('#dsidx-search-location').attr('name') != 'idx-q-ZipCodes<0>') {
			$('#dsidx-search-location').attr('name', 'idx-q-ZipCodes<0>');;
		}

		// serialize form data
		var params = $(this).find("input,select").filter( function() { return $(this).val() != ''; } ).serialize();

		// prep search path by sending to directly to /idx
		var idx_pos = window.location.pathname.indexOf('/idx');
		var path;
		if (idx_pos >= 0) {
		    path = window.location.pathname.slice(0, idx_pos + 5);
	    }else{
		    path = window.location.pathname + 'idx/';
        }

		// check for sort parameters
		var sc = getParameterByName(encodeURIComponent('idx-d-SortOrders<0>-Column'));
		var sd = getParameterByName(encodeURIComponent('idx-d-SortOrders<0>-Direction'));
		if (sc != '') {
			params += '&' + encodeURIComponent('idx-d-SortOrders<0>-Column') + '=' + encodeURIComponent(sc);
		}
		if (sd != '') {
			params += '&' + encodeURIComponent('idx-d-SortOrders<0>-Direction') + '=' + encodeURIComponent(sd);
		}
...
dsidx.results = function() {
	var $ = jQuery;
	var mapInitialized;
	var mapControlIcon = 'https://api-idx.diversesolutions.com/images/dsIDXpress/icons/map.png';
	var mapControlLink, mapControlShowText;

	function initializeMap() {
	    if (mapInitialized) return;
	    var mapDiv = $('#dsidx-map').get(0);
		var results = dsidx.dataSets.results;
		var bounds = new google.maps.LatLngBounds();
		var map, marker, result;
		var markers = [];
		var markerOrigin = new google.maps.Point(0, 0);
		var markerSize = new google.maps.Size(21, 26);
		var shadow = new google.maps.MarkerImage('https://api-idx.diversesolutions.com/images/dsIDXpress/markers/generic-shadow.png', new google.maps.Size(35, 28), markerOrigin, new google.maps.Point(11, 28));
		var normalMarker = new google.maps.MarkerImage('https://api-idx.diversesolutions.com/images/dsIDXpress/markers/short-single-house.png', markerSize, markerOrigin);
		var activeMarker = new google.maps.MarkerImage('https://api-idx.diversesolutions.com/images/dsIDXpress/markers/short-single-house-active.png', markerSize, markerOrigin);
		var soldMarker = new google.maps.MarkerImage('https://api-idx.diversesolutions.com/images/dsIDXpress/markers/short-single-house-sold.png', markerSize, markerOrigin);
		var startZoom = 8;
		
		for (var i = 0, l = results.length; i < l; ++i) {
			if (results[i].Latitude == -1 || results[i].Latitude == 0 || !results[i].Address)
				continue;

			bounds.extend(new google.maps.LatLng(results[i].Latitude, results[i].Longitude));
		}

		if (results.length == 0 && dsidx.mapStart) {
			bounds.extend(new google.maps.LatLng(dsidx.mapStart.latitude, dsidx.mapStart.longitude));
			startZoom = dsidx.mapStart.zoom;
		}
		...
		for (var i = 0, l = results.length; i < l; ++i) {
			result = results[i];
			if (result.Latitude == -1 || result.Latitude == 0 || !result.Address)
			    continue;
			...
			if(result.ListingStatusID == 4){
			    marker.icon = soldMarker;
			}
			markers.push(marker);

			// needed to close over result var
			(function(listing) {
				google.maps.event.addListener(marker, 'click', function() {
					window.location = dsidx.idxActivationPath + listing.PrettyUriForUrl;
				});
				google.maps.event.addListener(marker, 'mouseover', function(event) {
					...
					var markerPosition = {
						left: Math.floor((worldCoordinate.x - worldCoordinateNW.x) * scale) + mapOffset.left,
						top: Math.floor((worldCoordinate.y - worldCoordinateNW.y) * scale) + mapOffset.top
					};

					mouseOverMarker(this, listing, markerPosition);
				});
				google.maps.event.addListener(marker, 'mouseout', function (event) {
				    if (result.ListingStatusID == 4) {
				        this.setIcon(soldMarker);
				    } else {
				        this.setIcon(normalMarker);
				    }
					mouseOutMarker(this, listing);
				});
			})(result);
		}
	...
	function mouseOverMarker(marker, listing, markerPosition) {
		var hoverContainer = $('#dsidx-map-hover');
		var dataContainer = hoverContainer.find('.dsidx-container');
		var addressLine = (listing.Address ? listing.Address : '<i>no address</i>') + ', ' + (listing.City ? listing.City : '<i>no city</i>');
		var bathVal = null;
		hoverContainer.css({ 'display': 'block', 'visibility': 'hidden', 'height': '', 'width': '' });
		dataContainer.find('.dsidx-address').html(addressLine);

		if (listing.ListingStatusID == '4') {
		    dataContainer.find('.dsidx-price').html(listing.SalePrice);
		    var elems = document.getElementsByTagName('*'), i;
		    for (i in elems) {
		        if ((" " + elems[i].className + " ").indexOf(" dsidx-price ") > -1) {
		            elems[i].style.color = '#FFCCCC';
		        }
		    }
		}
		else {
		    dataContainer.find('.dsidx-price').html(listing.Price);
		}		
		dataContainer.find('.dsidx-beds').html(listing.BedsTotal == '0' ? '' : '<span>' + listing.BedsTotal + '</span> beds')
	    /**
        DIV-14948: If value in BathsTotal property is not equals to BathsShortString then use "BathsShortString". 
        This is just a work around , as actually the BathsTotal should always return correct value.
        */
		if (listing.BathsShortString.includes("baths")) {
		    bathVal = listing.BathsShortString.split(" ");
		}
		if (bathVal !== null && bathVal.length > 0 && bathVal[0] !== listing.BathsTotal)
		    dataContainer.find('.dsidx-baths').html(bathVal[0] === '0' ? '' : '<span>' + bathVal[0] + '</span> baths');
		else
		    dataContainer.find('.dsidx-baths').html(listing.BathsTotal === '0' ? '' : '<span>' + listing.BathsTotal + '</span> baths');
		dataContainer.find('.dsidx-sqft').html(listing.ImprovedSqFt == '0' ? '' : '<span>'+ listing.ImprovedSqFt + '</span> sqft');

		if (listing.ListingAttributionText)
			dataContainer.find('.dsidx-atribution').html(listing.ListingAttributionText);
		else
			dataContainer.find('.dsidx-atribution').css({ 'display': 'none' });

		if (listing.PhotoCount > 0)
		    //dataContainer.find('.dsidx-photo').html('<img src="' + listing.PhotoUriBase + '0-thumb.jpg" />');
		    dataContainer.find('.dsidx-photo').html('<img src="' + listing.PhotoUriBase + listing.PhotoFileName + '" />');
		else
			dataContainer.find('.dsidx-photo').html('<img src="https://api-idx.diversesolutions.com/images/dsIDXpress/no-photos-available-w100.jpg" />');

		if (listing.IdxIconUri)
			dataContainer.find('.dsidx-icon-container').html('<img src="' + listing.IdxIconUri + '" />');
		else
			dataContainer.find('.dsidx-icon-container').html('');
			
		hoverContainer
			.css({ visibility: 'visible' });

		var data_dimensions = { width: dataContainer.outerWidth(), height: dataContainer.outerHeight()};

		dataContainer
			.children('.dsidx-map-hover-divet').css({ top: (data_dimensions.height / 2 - 5.5) + 'px' });

		hoverContainer
			//.css({ 'height': data_dimensions.height + 'px', 'width': data_dimensions.width + 10 + 'px' })
			.css({ left: markerPosition.left + 20 + 'px', top: markerPosition.top - 87 + 'px' })
			.attr('mls-number', listing.MlsNumber)
		;
	}
	function mouseOutMarker(marker, listing) {
		var hoverContainer = $('#dsidx-map-hover');

		if (hoverContainer.attr('mls-number') == listing.MlsNumber)
			hoverContainer.css({ display: 'none' });
	}

	$('#dsidx .dsidx-sorting-control select').change(function() {
		var desiredSort = this.value.split('|');
		var currentQuery = window.location.search.substring(1).split('&');
		var newSortColumn = 'idx-d-SortOrders%3C0%3E-Column=' + encodeURIComponent(desiredSort[0]);
		var newSortDirection = 'idx-d-SortOrders%3C0%3E-Direction=' + encodeURIComponent(desiredSort[1]);
		var sortColumnReplaced, sortDirectionReplaced;
		var newQuery = [];
		var newUrl;
		...
		for (var key in currentQuery) {
			...
			var param = currentQuery[key];
			if (/idx-d(irective)?\-SortOrders.+Column/.test(param)) {
				newQuery.push(newSortColumn);
				sortColumnReplaced = true;
			} else if (/idx-d(irective)?\-SortOrders.+Direction/.test(param)) {
				newQuery.push(newSortDirection);
				sortDirectionReplaced = true;
			} else if (param) {
				newQuery.push(param);
			}
		}

		if (!sortColumnReplaced)
			newQuery.push(newSortColumn);
		if (!sortDirectionReplaced)
			newQuery.push(newSortDirection);
	...
	$('.dsidx-tabs.dsidx-result-tabs .dsidx-tab').click(function(){
		var tab = $(this);

		$('.dsidx-tabs.dsidx-result-tabs .dsidx-tab').addClass('dsidx-tab-disabled');
		tab.removeClass('dsidx-tab-disabled');
		
		eraseCookie("dsidx_listings_tab");

		if(tab.hasClass('dsidx-tab-list')) {
			$('#dsidx').removeClass('dsidx-results-map');
			$('#dsidx').removeClass('dsidx-results-grid');
		} else if(tab.hasClass('dsidx-tab-map')) {
			createCookie("dsidx_listings_tab", "dsidx-tab-map");

			$('#dsidx').addClass('dsidx-results-map');
			$('#dsidx').removeClass('dsidx-results-grid');

			initializeMap();
		} else if(tab.hasClass('dsidx-tab-grid')) {
			$('#dsidx').removeClass('dsidx-results-map');
			$('#dsidx').addClass('dsidx-results-grid');

			createCookie("dsidx_listings_tab", "dsidx-tab-grid");
		}
	});

	var current_tab = readCookie("dsidx_listings_tab");
	if (!current_tab) {
		if (dsidx.wp_options.ResultsDefaultState == "listmap" || dsidx.wp_options.ResultsMapDefaultState == "open") current_tab = "dsidx-tab-map";
		if (dsidx.wp_options.ResultsDefaultState == "grid") current_tab = "dsidx-tab-grid";
	}
	if(current_tab) $('.dsidx-tabs.dsidx-result-tabs .' + current_tab).click();
};
...
dsidx.resultsWidget = function() {
	...
	function displayContainer(container, panelClassToShow) {
		return container
			.children('.dsidx-panel').css({display: 'none'}).end()
			.children('.dsidx-' + panelClassToShow).css({display: 'block'})
		;
	}
	function displayList(containerId) {
		var container = containerId ? $('#' + containerId) : $(this).parents('.dsidx-results-widget');
		var panel = displayContainer(container, 'list');
	}
	function displayMap(containerId) {
		var container = containerId ? $('#' + containerId) : $(this).parents('.dsidx-results-widget');
		var panel = displayContainer(container, 'map');
		var results = dsidx.dataSets[containerId ? containerId : container.get(0).id];
		var tinyDetailsArea = panel.children('.dsidx-results-widget-map-details');
		var bounds = new google.maps.LatLngBounds();
		var map, marker, result;
		var markers = [];
		var markerOrigin = new google.maps.Point(0, 0);
		...
		map = new google.maps.Map(panel.children('.dsidx-container').get(0), {
			zoom: 14,
			center: bounds.getCenter(),
			mapTypeId: google.maps.MapTypeId.ROADMAP,
			mapTypeControl: false,
		...
		map.fitBounds(bounds);
		tinyDetailsArea.html('');

		setTimeout(function() {
		    for (var i = 0, l = results.length; i < l; ++i) {
		        result = results[i];
		        console.log(result);
		        if (result.Latitude == -1 || result.Latitude == 0 || result.ShortDescription.indexOf('MLS #') == -1)
		            continue;
		        ...
		        (function(instanceResult) {
		            google.maps.event.addListener(marker, 'click', function() {
		                window.location = dsidx.idxActivationPath + instanceResult.PrettyUriForUrl;
		            });
		            google.maps.event.addListener(marker, 'mouseover', function () {
		                for (var i = 0, l = markers.length; i < l; ++i)
		                    markers[i].setIcon(normalMarker);
		                this.setIcon(activeMarker);
		                var mlsNum = instanceResult.MlsNumber;
		                var listingHtml = $(".dsidx-expanded.dsidx-panel [data-mls_id='" + mlsNum + "']").clone();
		                tinyDetailsArea.html(listingHtml);
		                $('.dsidx-results-widget-map-details .featured-listing').hide().fadeIn('fast');
		            });
		        })(result);
		    }
		    if(markers.length > 0){
	...
	function displaySlideshow(containerId) {
		var container = containerId ? $('#' + containerId) : $(this).parents('.dsidx-results-widget');
		var panel = displayContainer(container, 'slideshow');
		var animating;
		var amountToScroll = panel.find('ul li').get(0).offsetHeight;
		
		panel.children('ul').get(0).scrollTop = 0;
		panel.children('.dsidx-move-up').css({color: '#999999'});
		panel.children('.dsidx-move-down').css({color: '#000000'});
		
		panel.children('.dsidx-move-up[data-observed!=true], .dsidx-move-down[data-observed!=true]').click(function() {
			if (animating)
				return;
			
			var clickedLink = $(this);
			var scrollDirection = clickedLink.hasClass('dsidx-move-up') ? '-' : '+';
			var ul = clickedLink.siblings('ul');
			
			animating = true;
			ul.animate({scrollTop: scrollDirection + '=' + String(amountToScroll) + 'px'}, 250, 'swing', function() {
				if (this.scrollTop == 0 || this.scrollTop + amountToScroll == this.scrollHeight)
					clickedLink.css({color: '#999999'}).siblings('.dsidx-slideshow-control').css({color: '#000000'});
				else
					clickedLink.siblings('.dsidx-slideshow-control').andSelf().css({color: '#000000'});
				animating = false;
			});
		}).attr('data-observed', 'true');
	}

	function displayExpanded(containerId) {
		var container = containerId ? $('#' + containerId) : $(this).parents('.dsidx-results-widget');
		var panel = displayContainer(container, 'expanded');
	}
	
	returnObj = {
		setInitialState: function(id, state) {
			if (!dsidx.dataSets[id].length)
				return;
	...
	$('.dsidx-results-widget .dsidx-controls').each(function() {
		$(this).append($('<a href="javascript:void(0)" data-panel="list">list</a>').click(function() { displayList.call(this) }).css(linkStyles))
		$(this).append($('<a href="javascript:void(0)" data-panel="slideshow">slideshow</a>').click(function () { displaySlideshow.call(this) }).css(linkStyles))
		$(this).append($('<a href="javascript:void(0)" data-panel="expanded">details</a>').click(function () { displayExpanded.call(this) }).css(linkStyles))
		$(this).append($('<a href="javascript:void(0)" data-panel="map">map</a>').click(function() { displayMap.call(this) }).css(linkStyles))
	});
...
dsidx.details = function() {
	...
	function initializeMap() {
		var mapContainer = $('.dsidx-details #dsidx-map');
		if (!mapContainer.length)
			return;

		var propertyPoint = new google.maps.LatLng(dsidx.details.latitude, dsidx.details.longitude);
		...
		var marker = new google.maps.Marker({
			...
			map: map,
			icon: 'https://api-idx.diversesolutions.com/images/dsIDXpress/markers/short-single-house.png'
		});
	}
	function initializeStopYelling() {
		var textEl = $('#dsidx-description');
		var text = textEl.text();

		if (text.length > 20 && text == text.toUpperCase())
			$('<a id="dsidx-quit-yelling" href="javascript:void(0)">(click here to stop the yelling)</a>').click(stopYelling).appendTo(textEl);
	}
	/* deprecated with JuiceBox implementation - leave in for backwards compatibility with older versions of dsidxpress */
	function initializePhotos() {
		$('#dsidx-tiny-photos a').dsidx_colorbox({
			transition: 'none',
			width: false,
			height: false
		});
		if (!dsidx.swfobject.hasFlashPlayerVersion('9.0.0'))
			$('#dsidx-primary-photo').click(function() { $('#dsidx-tiny-photos a').first().click(); return false; });
	}
	function initializeSlideshow() {
    	if (dsidx.details.photoCount < 2)
		    return;

        // Added check for user setting for juicebox use
		if (!dsidx.details.useJuiceBoxSlideShow)
		    return;

		var encodedPhotoBase;
		if (dsidx.getVersionId() < 10000.08)
			encodedPhotoBase = encodeURIComponent(dsidx.details.photoUriBase);
		else if (dsidx.details.photoUriBase)
		    encodedPhotoBase = encodeURIComponent(dsidx.details.photoUriBase.substr(7));

		if (typeof modifiedImageCount !== 'undefined') {
		    dsidx.details.photoCount = modifiedImageCount;
		}
		
		var xmlBasePath = dsidx.pluginUrl + 'client-assist.php?action=SlideshowParams';
		if (dsidx.useWPAjax) {
		    xmlBasePath = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=SlideshowParams';
		}
		var xmlPath = xmlBasePath + '&count=' + String(dsidx.details.photoCount) + '&uriBase=' + encodedPhotoBase;

		dsidx.swfobject.embedSWF(
			dsidx.pluginUrl + 'assets/slideshowpro.swf',
			'dsidx-photos',
			'100%',
			'400',
			'9.0.0',
			null,
			{ paramXMLPath: encodeURIComponent(xmlPath), slideshowCaption: dsidx.details.fullSizePhotosCaption, contentScale: 'Crop to Fit All' },
			{ allowFullScreen: 'true', wmode: 'transparent' }
		);
	}
	/* end deprecated functions */

	function stopYelling() {
		var textEl = $('#dsidx-description-text');
		var text = textEl.text();

		text = text.toLowerCase();
		text = text.replace(/(?:^|[!?.])\s*([a-z])/g, function($1) { return $1.toUpperCase(); });
		textEl.text(text);
		$('#dsidx-quit-yelling').css({ display: 'none' });
	}

	function InitializeForm() {
		$('.dsidx-contact-form-schedule-date-row').hide();
		$('.dsidx-contact-form-schedule-yesno').attr('checked', false);

		$('.dsidx-contact-form-schedule-yesno').click(function() {
			if (!$('.dsidx-contact-form-schedule-yesno').attr('checked'))
				$('.dsidx-contact-form-schedule-date-row').hide();
			else
				$('.dsidx-contact-form-schedule-date-row').show();
		});

		$('#dsidx-button-schedule-showing').click(function() {
			$('.dsidx-contact-form-schedule-yesno').attr('checked', true)
			$('.dsidx-contact-form-schedule-date-row').show();
		...
		$('#dsidx-button-contact-agent').click(function() {
			dsidx.details.ScrollToForm();
			return false;
		});
		
		$('#dsidx-contact-form-submit').click(sendContactForm);
		$('#emailAddress').blur(checkIsOptIn);
	}

	function checkIsOptIn(e) {
	    var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=isOptIn';
	    if (dsidx.useWPAjax) {
	        ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=isOptIn';
	    }
	    $.ajax({
	        type: 'GET',
	        url: ajaxUrl,
	        ...
	        success: function (response) {
	            if (response.isOptIn) {
	                $('#dsidx-isoptin').val(response.isOptIn);
	                $('#dsidx-optin').val(!response.isOptIn);
	            } else {
	                $('#dsidx-isoptin').val(response.isOptIn);
	                $('#dsidx-optin').val(!response.isOptIn);
	            }
	...
	function sendContactForm() {
		if ($(".dsidx-contact-form #firstName").length && $(".dsidx-contact-form #lastName").length) {
			if (!validateDetailViewCF()) return false;
			//Set Cookies
			setCookie("ds_Name", $('.dsidx-contact-form #firstName').val() + ' ' + $('.dsidx-contact-form #lastName').val(), 30, '/');
			setCookie("ds_Email", $('.dsidx-contact-form #emailAddress').val(), 30, '/');
			setCookie("ds_Phone", $('.dsidx-contact-form #phoneNumber').val(), 30, '/');
		}
		else {
			if (!validateNotFoundViewCF()) return false;
			setCookie("ds_Name", $('.dsidx-contact-form #name').val(), 30, '/');
			setCookie("ds_Email", $('.dsidx-contact-form #emailAddress').val(), 30, '/');
		}
		
		$('.dsidx-contact-form #dsidx-contact-form-submit').attr('disabled', 'disabled');
		$('.dsidx-contact-form #dsidx-contact-form-submit').val('Sending...');

		var dsidxAjaxUrl = dsidx.pluginUrl + 'client-assist.php';

		if (dsidx.useWPAjax) {
		    $('.dsidx-contact-form input[name="action"]').val('dsidx_client_assist');
		    $('.dsidx-contact-form').append('<input type="hidden" name="dsidx_action" value="ContactForm" />');
		    dsidxAjaxUrl = dsidxAjaxHandler.ajaxurl;
		}
		var values = $('.dsidx-contact-form input, .dsidx-contact-form textarea, .dsidx-contact-form select').serialize();

	    $.post(dsidxAjaxUrl, values, function (data) {
		    if (data.Error) {
		        $('.dsidx-contact-form .dsidx-contact-form-message').html('Problem submitting request.').addClass('error');
		    }
		    else {
		        $('.dsidx-contact-form .dsidx-contact-form-message').html('Sent!').addClass('success');
		        if (!$('.dsidx-contact-form-schedule-yesno').attr('checked')) {
		            dsidx.trackEvent('Contact', '/IDX/Request More Info');
		        }
		        else {
		            dsidx.trackEvent('Contact', '/IDX/Schedule a Showing');
		        }
		        dsidx.trackPageview('/IDX/Send Contact Form');
		        if (typeof (DSListTrac) != 'undefined' && typeof (DSListTracData) != 'undefined') {
		            DSListTrac.trackEvent('lead', DSListTracData);
		        }
		    }
			$('.dsidx-contact-form #dsidx-contact-form-submit').removeAttr('disabled');
			$('.dsidx-contact-form #dsidx-contact-form-submit').val('Submit');
		}, "json").error( function (data) {
			$('.dsidx-contact-form .dsidx-contact-form-message').html('Problem submitting request.');
			$('.dsidx-contact-form #dsidx-contact-form-submit').removeAttr('disabled');
			$('.dsidx-contact-form #dsidx-contact-form-submit').val('Submit');
		});
	}
	/*Contact Form Validators*/
	function validateDetailViewCF() {
		var err = false, errs = new Array(), index = 0, focus = null;
		
		if ($('.dsidx-contact-form .dsidx-contact-form-first-name').val() == '') {
			err = true;
			errs[index] = "First Name";
			focus = $('.dsidx-contact-form .dsidx-contact-form-first-name');
			index++;
		}
		
		if ($('.dsidx-contact-form .dsidx-contact-form-last-name').val() == '') {
			err = true;
			errs[index] = "Last Name";
			focus = (focus === null) ? $('.dsidx-contact-form .dsidx-contact-form-last-name') : focus;
			index++;
		}

		if ($('.dsidx-contact-form .dsidx-contact-form-phone-number').hasClass('required') || $('.dsidx-contact-form .dsidx-contact-form-phone-number').val() != '') {
			var phoneVal = $('.dsidx-contact-form .dsidx-contact-form-phone-number').val().replace(/\D/g, '');
			...
			if (phoneVal.length >= 10 && phoneVal.length <= 15) {
				if (phoneVal[0] == '0' || phoneVal[0] == '1' || phoneVal[1] == '9' || phoneVal.substr(0, 3) == '555' || phoneVal[3] == '0' || phoneVal[3] == '1') {
					errs[index] = 'Phone Number';
				}
			} else {
				errs[index] = 'Phone Number';
			}
			if (errs[index] == 'Phone Number') {
				err = true;
				focus = (focus === null) ? $('.dsidx-contact-form .dsidx-contact-form-phone-number') : focus;
				index++;
			}
		}

		if ($('.dsidx-contact-form .dsidx-contact-form-email-address').val() == '') {
			err = true;
			errs[index] = "Email Address";
			focus = (focus === null) ? $('.dsidx-contact-form .dsidx-contact-form-email-address') : focus;
			index++;
		}
	...
	function validateNotFoundViewCF() {
		var err = false, errs = new Array(), index = 0, focus = null;
		
		if ($('.dsidx-contact-form #name').val() == '') {
			err = true;
			errs[index] = "Last Name";
			focus = (focus === null) ? $('.dsidx-contact-form #name') : focus;
			index++;
		}

		if ($('.dsidx-contact-form #emailAddress').val() == '') {
			err = true;
			errs[index] = "Email Address";
			focus = (focus === null) ? $('.dsidx-contact-form #emailAddress') : focus;
			index++;
		}
	...
	/*End Contact Form Validators*/
	function loadSimilarProperties () {
		if (!$('#dsidx-similar-listings').length) return;
		if (dsidx.bot) return;

		var ajaxUrl = dsidx.pluginUrl + 'client-assist.php';
		var ajaxAction = 'LoadSimilarListings';

        if (dsidx.useWPAjax) {
            ajaxUrl = dsidxAjaxHandler.ajaxurl;
            ajaxAction = 'dsidx_client_assist';
		}
		$.post(ajaxUrl,
			{   'action': ajaxAction,
			...
			function(d) { $('#dsidx-similar-listings').html(d); });
	}

	function loadSoldProperties () {
		if (!$('#dsidx-sold-listings').length) return;
		if (dsidx.bot) return;

		var ajaxUrl = dsidx.pluginUrl + 'client-assist.php';
		var ajaxAction = 'LoadSoldListings';
		...
		$.post(ajaxUrl,
			{
				...
				'SearchSetupID': dsidx.details.ssid, 
				'PropertyID': dsidx.details.pid },
			function(d) { $('#dsidx-sold-listings').html(d); });
	}

	function loadSchools () {
		if (!$('#dsidx-ajax-schools').length) return;
		if (dsidx.bot) return;

		var ajaxUrl = dsidx.pluginUrl + 'client-assist.php';
		var ajaxAction = 'LoadSchools';
		...
		$.post(ajaxUrl,
			{
				...
				'PropertyID': dsidx.details.pid },
			function(d) { $('#dsidx-ajax-schools').html(d);	});
	}

	function loadDistricts () {
		if (!$('#dsidx-ajax-districts').length) return;
		if (dsidx.bot) return;

		var ajaxUrl = dsidx.pluginUrl + 'client-assist.php';
		var ajaxAction = 'LoadDistricts';
		...
		$.post(ajaxUrl,
			{
				...
				'spatial': dsidx.details.latitude != -1 && dsidx.details.latitude != 0 ? 'true' : 'false',
				'PropertyID': dsidx.details.pid },
			function(d) { $('#dsidx-ajax-districts').html(d) });
	}

	function loadCookieValues() {
		if($('.dsidx-contact-form').length == 0) return;
		if ($(".dsidx-contact-form #firstName").length && $(".dsidx-contact-form #lastName").length) { //details view contact form
			var Name = loadVisitorInfoFromCookie("ds_Name");
			if (Name) {
				var firstName = Name.split(' ')[0];
				var lastName = Name.split(' ')[1];
			}
			var Phone = loadVisitorInfoFromCookie("ds_Phone");
			var Email = loadVisitorInfoFromCookie("ds_Email");
			if ($(".dsidx-contact-form #firstName").val().length == 0 && firstName) $(".dsidx-contact-form #firstName").val(firstName);
			if ($(".dsidx-contact-form #lastName").val().length == 0 && lastName) $(".dsidx-contact-form #lastName").val(lastName);
			if ($(".dsidx-contact-form #phoneNumber").val().length == 0 && Phone) $(".dsidx-contact-form #phoneNumber").val(Phone);
			if ($(".dsidx-contact-form #emailAddress").val().length == 0 && Email) $(".dsidx-contact-form #emailAddress").val(Email);
		}
		else { //404 contact form
			var Name = loadVisitorInfoFromCookie("ds_Name");
			var Email = loadVisitorInfoFromCookie("ds_Email");
			if ($(".dsidx-contact-form #name").val().length == 0 && Name) $(".dsidx-contact-form #name").val(Name);
			if ($(".dsidx-contact-form #emailAddress").val().length == 0 && Email) $(".dsidx-contact-form #emailAddress").val(Email);
		}
	}

	function loadVisitorInfoFromCookie (name) {
	...
	function notFoundSearchForm() { // 404 page
		$("#not-found-search-form").submit(function() {
			if ($("#dsidx-not-found-location-field").val().trim().match(/^\d{5}$/) && $('#dsidx-not-found-location-field').attr('name') != 'idx-q-ZipCodes<0>') {
				$('#dsidx-not-found-location-field').attr('name', 'idx-q-ZipCodes<0>');;
			}
		});
	}

	function trackDetailsPageView() {
	    dsidx.trackPageview('/IDX/View Property Details');
	    if (typeof (DSListTrac) != 'undefined' && typeof (DSListTracData) != 'undefined') {
	        DSListTrac.trackEvent('view', DSListTracData);
	    }
	}

	function trackVirtualTourView() {
	    if (typeof (DSListTrac) != 'undefined' && typeof (DSListTracData) != 'undefined') {
	        DSListTrac.trackEvent('vTour', DSListTracData);
	    }
	}

	function trackGalleryExpand() {
	    if (typeof (DSListTrac) != 'undefined' && typeof (DSListTracData) != 'undefined') {
	        DSListTrac.trackEvent('gallery', DSListTracData);
	    }
	}

	function initExtraClickTracks() {
	    $('#dsidx-virtual-tour-container a').on('click', function () {
	        if (typeof (DSListTrac) != 'undefined' && typeof (DSListTracData) != 'undefined') {
	            DSListTrac.trackEvent('vTour', DSListTracData);
	        }
	    });
    ...
    function printListing(mlsNumber) {
        jQuery('#dsidx-profile-sharing-button').removeClass('Selected'); jQuery('#dsidx-profile-share').css('display', 'none');
        var printUrl = dsidx.pluginUrl + 'client-assist.php?action=PrintListing&MlsNumber=' + mlsNumber;
        if (dsidx.useWPAjax) {
            printUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=PrintListing&MlsNumber=' + mlsNumber;
        }
        window.open(printUrl, '_blank');
    }

	initializeMap();
	initializeStopYelling();
	initializePhotos();
	InitializeForm();
	initializeSlideshow();
	loadCookieValues();
	...
	loadSchools();
	notFoundSearchForm();
	trackDetailsPageView();
	initExtraClickTracks();

	return {
	    pid: dsidx.details.pid,

	    LaunchLargePhoto: function (index, count) {
	        $('#dsidx-tiny-photos a').eq(index).click();
	    },

	    ScrollToForm: function () {
	        var scrollTopDst = $('#dsidx-contact-form-header').offset().top;

	        // webkit needs to animate document.body while FF and IE animate document.documentElement (html). it doesn't
	        // currently cause errors when either of them fail, but we'll wrap each call in a try / catch just to be safe

	        try {
	            $(document.documentElement).animate({'scrollTop': scrollTopDst}, 400);
	        } catch (ex) { }

	        try {
	            $(document.body).animate({'scrollTop': scrollTopDst}, 400);
	        } catch (ex) { }
	    },

	    GetConfigUrl: function(){
	        if (typeof (dsidxAjaxHandler) != 'undefined') {
	            return dsidxAjaxHandler.ajaxurl + "?action=dsidx_client_assist&dsidx_action=GetPhotosXML&pid=" + dsidx.details.pid;
	        }
	    ...
	    printListing: function (mlsNumber) {
	        printListing(mlsNumber);
	    }
	};
};

dsidx.detailsWidget = function() {
	var $ = jQuery, widgetData = [];

	$('.dsidx-widget-single-listing .dsidx-widget-single-listing-slideshow-next').click(function(e) {
		var $p = $(this).parents('.dsidx-widget');

		if (typeof widgetData[$p.attr('id')] === 'undefined') {
			initInstance($p);
		}

		var next = (widgetData[$p.attr('id')].current < widgetData[$p.attr('id')].maximum) ? widgetData[$p.attr('id')].current + 1 : 1;

		scrollTo($p.attr('id'), next);
	});

	$('.dsidx-widget-single-listing .dsidx-widget-single-listing-slideshow-prev').click(function(e) {
		var $p = $(this).parents('.dsidx-widget');
		...
		var prev = (widgetData[$p.attr('id')].current > 1) ? widgetData[$p.attr('id')].current - 1 : widgetData[$p.attr('id')].maximum;

		scrollTo($p.attr('id'), prev);
	});

	var initInstance = function(el) {
		widgetData[el.attr('id')] = {
			'element': el,
			'current': 1,
			'maximum': el.find('.dsidx-widget-single-listing-photo').length,
			'width':   el.find('.dsidx-widget-single-listing-photo').width(),
			'height':  el.find('.dsidx-widget-single-listing-photo').height()
		}
	}

	var scrollTo = function (id, target) {
	    var nextItem = widgetData[id].element.find('.dsidx-widget-single-listing-photo:eq(' + (target - 1) + ')');
	    if (nextItem.find('img').length == 0 && nextItem.find('.img-placeholder').length != 0) {
	        var placeholder = nextItem.find('.img-placeholder:eq(0)');
	        var newImg = $('<img>', { src: placeholder.attr('data-img-src'), alt: placeholder.attr('data-img-alt') });
	        placeholder.parent().append(newImg);
	        placeholder.remove();
	    }
		var nextEdge = (target * widgetData[id].width) - widgetData[id].width;
		widgetData[id].current = target;
		widgetData[id].element.find('.dsidx-widget-single-listing-slideshow').css({'margin-left': '-' + nextEdge + 'px'});
	}
};


// this doesn't use the dsidx.activate() system since we don't want any explicit script tags in our shortcode.
// since this will be run every time the script is included but won't necessarily be relevant every time it's included,
// let's keep this short and sweet.
/**
    We have an issue when multiple shortcode items are placed in the same page.
    It basically the colorbox init as soon as the first one renders. This means subsequent colorbox galleries never get inited.
    To solve this, we assign each shortcode item a unique id and initialize them directly by calling the initColorbox function inline.
**/
dsidx.detailsShortcode = {
    initColorbox: function (SCTimestamp) {
        var $ = jQuery;
        var ShortCodeDivID = $('#dsidx-shortcode-item-' + SCTimestamp);
        $('a.dsidx-photo-thumb', ShortCodeDivID).dsidx_colorbox({
            transition: 'none',
            width: false,
            height: false
        });
...
dsidx.sharing = function () {
	var sharingDialogWidth = 145;
	var emailDialogWidth = 265;
	...
	jQuery('#dsidx-share').dialog({
		...
		width: sharingDialogWidth,
		minHeight: 90,
		closeText: ''
	});
	jQuery('#dsidx-share-form-submit').click(sendShareToEmail);

	function sendShareToEmail() {
	    if (jQuery('#captchaAnswer').val() != jQuery('#captchaAnswerKey').val()) {
	        jQuery('#dsidx-share-email td.dsidx-share-email-message').html('Error: You must answer the "Are You Human" math question correctly');
	        return;
	    }
        ...
        var values = jQuery('#dsidx-share-email-form input, #dsidx-share-email-form textarea, #dsidx-share-email-form select').serialize();

	    jQuery.post(ajaxUrl, values, function (data) {
			jQuery('#dsidx-share-email .dsidx-submit').removeAttr('disabled');
			jQuery('#dsidx-share-email .dsidx-submit').val('Send');
			console.log(values);
			if (data.Error) {
				jQuery('#dsidx-share-email td.dsidx-share-email-message').html('Error: ' + data.Message);
			} else {
				...
				if (typeof (DSListTrac) != 'undefined' && typeof (DSListTracData) != 'undefined') {
				    DSListTrac.trackEvent('share', DSListTracData);
				}
	...
	return {
		Click: function (obj, twitter, facebook, propertyID) {
			var button = jQuery(obj);

			jQuery(document).click(function (e) {
				var target = e.target;
				if (!jQuery(target).is('#dsidx-profile-sharing-button') && !jQuery(target).parents().is('#dsidx-profile-sharing-button') && !jQuery(target).is('#dsidx-profile-share') && !jQuery(target).parents().is('#dsidx-profile-share')) {
					var button = jQuery('#dsidx-profile-sharing-button');
					var panel = jQuery('#dsidx-profile-share');
			...
			if (twitter != null)
				dsidx.sharing.twitterShareUrl = twitter;
			if (facebook != null)
				dsidx.sharing.facebookShareUrl = facebook;
			if (propertyID != null)
				jQuery('#dsidx-share-email-propertyid').val(propertyID);

			var panel = jQuery('#dsidx-profile-share');
			panel.css('display', 'block');

			jQuery('#dsidx-profile-sharing-button').removeAttr('onclick'); //have to explicitly set the inline onclick to null
		...
		ToggleShare: function () {
			...
			if (button.hasClass('Selected')) {//it's open /close it
				button.removeClass('Selected');
				panel.css('display', 'none');
			}
			else {
		...
		ShareToTwitter: function () {
			var w = window.open(dsidx.sharing.twitterShareUrl, '_blank', 'width=800,height=600,location=0,menubar=0,toolbar=0,scrollbars=1');
			w.focus();
		...
		ShareToFacebook: function () {
			var w = window.open(dsidx.sharing.facebookShareUrl, '_blank', 'width=800,height=600,location=0,menubar=0,toolbar=0,scrollbars=1');
			w.focus();
		...
		ShareToEmail: function () {
		    ...
		    jQuery('#captchaQuestion').html(random1 + " + " + random2 + " = ");
		    jQuery('#captchaAnswerKey').val(random1 + random2);
		    jQuery('#captchaAnswer').value = '';

			var note = "Your friend has sent you a real estate property to look at. Here is some brief information about the property and any notes that have been sent along with the property."

			jQuery('#dsidx-share-choose').hide();
			jQuery('#dsidx-share-email').show();

			jQuery('#dsidx-share-email td.dsidx-share-email-message-row').hide();
			jQuery('#dsidx-share').dialog('option', 'width', emailDialogWidth);
		},

		CancelShareToEmail: function () {
			jQuery('#dsidx-share-choose').show();
			jQuery('#dsidx-share-email').hide();
			jQuery('#dsidx-share').dialog('option', 'width', sharingDialogWidth);
		}
	};
...
dsidx.auth = function () {
	...
	var init = function(){
		$('#dsidx-login form').submit(login_submit);
		$('#dsidx-register form').submit(register_submit);
		$('#dsidx-forgotpassword-confirm form').submit(forgotpassword_submit);
		$('#dsidx-passwordreset form').submit(passwordreset_submit);
		$('#dsidx-register-email').blur(checkIsOptIn);
		// Detect hash tag.
		var hash = window.location.hash.substring(1).toLowerCase();
		if (hash == 'showpasswordreset') {
			var querystring = getQuerystring();
			$('#dsidx-passwordreset-resettoken').val(querystring["reset"]);

			// Show password reset modal.
			dsidx.auth.PasswordReset();
		}

		if (typeof canUseSSO === 'undefined') {
			return;
		}
	...
	var validateLogout = function () {
		...
		var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=ValidateLogout';
		if (dsidx.useWPAjax) {
		    ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=ValidateLogout';
		}
		$.ajax({
			type: 'POST',
			url: ajaxUrl,
			...
			success: function (response) {
				if (response.success) {
					eraseCookie('dsidx-visitor-public-id');
					eraseCookie('dsidx-visitor-auth');
	...
	var promptLogin = function () {
		...
		if (window.location.href.indexOf('promptLogin=1') >= 0){
			dsidx.auth.Login();
		} else if (window.location.href.indexOf('promptSignUp=1') >= 0){
			dsidx.auth.Register();
		}
	};

	var checkSsoCookie = function (promptLogin) {
		// already authenticated
		if (dsidx.visitor) {
			return;
		}
		
		// check if sso cookie is set
		var url = getSsoPath(null, promptLogin);
		window.location = url;
	};
	
	var getSsoPath = function (auth, promptLogin) {
	    var ajaxBaseUrl = dsidx.pluginUrl + 'client-assist.php?action=';
	    if (dsidx.useWPAjax) {
	        ajaxBaseUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=';
	    }
	    var returnUrl = encodeURIComponent(ajaxBaseUrl + 'SsoAuthenticated&origin=' + encodeURIComponent(window.location.href));
	    if (dsidx.pluginUrl.indexOf(window.location.origin) == -1 && !dsidx.useWPAjax) {
	        // if the pluginUrl is a relative path, add the host to the front
	        returnUrl = encodeURIComponent(window.location.origin) + returnUrl;
	    }
	    var url = ajaxBaseUrl + 'SsoAuthenticate&' + 'requester.AccountID=' + dsidx.profile.accountID + '&query.SearchSetupID=' + dsidx.profile.ssid + '&requester.ApplicationProfile=' + dsidx.profile.appProfile + '&returnUrl=' + returnUrl;

		if (auth) {
		    var queryParams = getQuerystring();
		    if (queryParams.indexOf('origin') != -1) {
		        url = decodeURIComponent(queryParams['origin']);
		    }
		    else {
		        url += '&authToken=' + auth;
		    }
		}
		if (promptLogin) url += '&promptLogin=1';

		return url;
	};

	var getSsoSignOutPath = function () {
	    var returnUrl = encodeURIComponent(window.location.href);
	    var baseUrl = dsidx.pluginUrl + 'client-assist.php?action=SsoSignout';
	    if (dsidx.useWPAjax) {
	        baseUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=SsoSignout';
	    }
	    var url = baseUrl + '&' + 'requester.AccountID=' + dsidx.profile.accountID + '&query.SearchSetupID=' + dsidx.profile.ssid + '&requester.ApplicationProfile=' + dsidx.profile.appProfile + '&returnUrl=' + returnUrl;
		return url;
	};

	var isSsoEnabled = function () {
		var cookie = readCookie('dsidx-visitor-auth-sso');
		if (cookie) return false;
		
		if (!dsidx.profile.ssoEnabled) {
			createCookie('dsidx-visitor-auth-sso', 'checked', 1);
			return false;
		}
	...
	var setSsoCookie = function (auth) {
	    var url = getSsoPath(auth);
		window.location = url;
	};

	var login_submit = function () {
	    var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=Login';
	    if (dsidx.useWPAjax) {
	        ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=Login';
	    }
		$.ajax({
			type: 'POST',
			url: ajaxUrl,
			data: $('#dsidx-login form').serialize(),
            dataType: 'json',
			success: function(response){
				$('#dsidx-login-submit').val('Login');
				if (response.Success) {
					var cookie = readCookie('dsidx-visitor-auth');
					if (canUseSSO) {
					    setSsoCookie(cookie);
					} else {
					    var path = getPathFromUrl();
					    var querystring = getQuerystringWithoutHashAndToken();
					    var queryParams = getQuerystring();
					    if (queryParams.indexOf('origin') != -1) {
					        window.location = decodeURIComponent(queryParams['origin']);
					    }
					    else if (querystring) {
					        // If querystring still exists after the cleanup, reload with the querystring.
					        // Otherwise, just use the full path.
					...
					$('#dsidx-login .dsidx-dialog-message').show().html(response.Message);
				}
			}
		});
		
		$('#dsidx-login-submit').val('Sending...');
	...
	var register_submit = function(){
		...
		$('#dsidx-register-referral').val(document.referrer);

		if(!emailPattern.test($('#dsidx-register-email').val()))
			validationMessage = "You must use a valid email.";

		var phoneVal = $('#dsidx-register-phone-number').val().replace(/\D/g, '');
		if (phoneVal[0] == 1) phoneVal = phoneVal.substr(1);
		if (phoneVal.length >= 10 && phoneVal.length <= 15) {
			if (phoneVal[0] == '0' || phoneVal[0] == '1' || phoneVal[1] == '9' || phoneVal.substr(0, 3) == '555' || phoneVal[3] == '0' || phoneVal[3] == '1') {
		...
		if(!$('#dsidx-register-password').val() || $('#dsidx-register-password').val() != $('#dsidx-register-confirm-password').val())
		    validationMessage = "Passwords do not match.";

		if (!$('#dsidx-register-first-name').val() || !$('#dsidx-register-last-name').val()) validationMessage = "First and last name are required.";

		if(validationMessage){
			$('#dsidx-register .dsidx-dialog-message').show().html(validationMessage);
			return false;
		}

		$('#dsidx-register .dsidx-dialog-message').hide();
		...
		$.ajax({
			type: 'POST',
			url: ajaxUrl,
			data: $('#dsidx-register form').serialize(),
            dataType: 'json',
			success: function (response) {
				$('#dsidx-register-submit').val('Register');
				if (response.Success) {
					...
					$('#dsidx-register .dsidx-dialog-message').show().html(response.Message);
				}
			}
		});

		$('#dsidx-register-submit').val('Sending...');

		return false;
	};

	var forgotpassword_submit = function () {
		var message = "";

		$('#dsidx-forgotpassword-referral').val(document.referrer);

		if (!isValidEmail($('#dsidx-forgotpassword-email').val()))
			message = "You must use a valid email address.";

		if (message) {
			$('#dsidx-forgotpassword-confirm .dsidx-dialog-message').show().html(message);
			return false;
		}

		$('#dsidx-forgotpassword-confirm .dsidx-dialog-message').hide();

		var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=LoginRecovery';
		if (dsidx.useWPAjax) {
		    ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=LoginRecovery';
		}
		...
		$.ajax({
			...
			dataType: 'json',
			url: ajaxUrl,
			data: $('#dsidx-forgotpassword-confirm form').serialize(),
			success: function (response) {
				$('#dsidx-forgotpassword-submit').removeAttr("disabled");
				$('#dsidx-forgotpassword-submit').val('Send');
				$('#dsidx-forgotpassword-confirm .dsidx-dialog-message').show().html(response.Message);
				$('#dsidx-forgotpassword-email').val('');
			},
			error: function (xhr, options, error) {
				$('#dsidx-forgotpassword-submit').removeAttr("disabled");
				$('#dsidx-forgotpassword-submit').val('Send');
				$('#dsidx-forgotpassword-confirm .dsidx-dialog-message').show().html('We apologize we encountered an error. Please try again in a few minutes.');
			}
		});

		$('#dsidx-forgotpassword-submit').attr("disabled", "disabled");
		$('#dsidx-forgotpassword-submit').val('Sending ...');

		return false;
	};

	var passwordreset_submit = function () {
		var validationMessage = "";
		var passwordText = $('#dsidx-passwordreset-password');
		var passwordConfirmText = $('#dsidx-passwordreset-confirmpassword');
		var messageText = $('#dsidx-passwordreset .dsidx-auth-buttons .dsidx-dialog-message');

		$('#dsidx-passwordreset-referral').val(document.referrer);

		if (!passwordText.val() && !passwordConfirmText.val())
			validationMessage = "Please fill in the passwords.";
		else if (!passwordText.val() || passwordText.val() != passwordConfirmText.val())
			validationMessage = "Please make sure passwords match.";

		if (validationMessage) {
			messageText.show().html(validationMessage);
			return false;
		}

		messageText.hide();

		var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=ResetPassword';
		if (dsidx.useWPAjax) {
		    ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=ResetPassword';
		}
		...
		$.ajax({
			...
			dataType: 'json',
			url: ajaxUrl,
			data: $('#dsidx-passwordreset form').serialize(),
			success: function (response) {
				// Open the login modal and show success message.
				dsidx.auth.Login(response.Message);
			},
			error: function (xhr, options, error) {
				$('#dsidx-forgotpassword-confirm .dsidx-dialog-message').show().html('We apologize we encountered an error. Please try again in a few minutes.');
			}
		...
		$('#dsidx-passwordreset-submit').val('Sending...');
	...
	function checkIsOptIn() {
		...
		$.ajax({
			type: 'GET',
			url: ajaxUrl,
			data: ({ email: $('#dsidx-register-email').val() }),
            dataType: 'json',
			success: function (response) {
	            if (response) {
	                if (response.isOptIn) {
	                    $('#dsidx-isoptin').val(response.isOptIn);
	                    $('#dsidx-reg_optout').attr('checked', 'checked');
	                    $('#dsidx-reg_optin').show();
	                    $('#dsidx-reg_optout').show();
	                    $('#opt-yes').show();
	                    $('#opt-no').show();
	                    $('#opt-url1').text(window.location.hostname);
	                    $('#dsidx-optin-email-label').show();
	                } else {
	                    $('#dsidx-isoptin').val(response.isOptIn);
	                    $('#dsidx-reg_optin').attr('checked', 'checked');
	                    $('#dsidx-reg_optin').hide();
	                    $('#dsidx-reg_optout').hide();
	                    $('#opt-yes').hide();
	                    $('#opt-no').hide();
	                    $('#dsidx-optin-email-label').hide();
	                }
	            } else {
	                $('#dsidx-isoptin').val(response.isOptIn);
	                $('#dsidx-reg_optin').attr('checked', 'checked');
	...
	function getQuerystring() {
		...
		var querystring = location.href.replace(location.hash, "");
	...
	function getQuerystringWithoutHashAndToken() {
		var url = window.location.href;

		// Short circuit if url doesn't have querystring.
		if (url.indexOf('?') === -1)
			return "";

		var urlWithoutHash = location.href.replace(location.hash, "");
		var querystring = urlWithoutHash.slice(urlWithoutHash.indexOf('?') + 1).split('&');

		// Reconstruct querystring without the RESET param.
		// This is done to keep any other possible parameters added by other functions.
		var newQuerystring = "";
	...
	var pub = {
		...
		CloseAll: function () {
			if ($('#dsidx-login').hasClass('ui-dialog'))
				$('#dsidx-login').dialog('close');

			if ($('#dsidx-register').hasClass('ui-dialog'))
				$('#dsidx-register').dialog('close');

			if ($('#dsidx-forgotpassword-confirm').hasClass('ui-dialog'))
				$('#dsidx-forgotpassword-confirm').dialog('close');

			if ($('#dsidx-passwordreset').hasClass('ui-dialog'))
				$('#dsidx-passwordreset').dialog('close');

			$('#dsidx-login .dsidx-dialog-message').hide();
			$('#dsidx-register .dsidx-dialog-message').hide();
			$('#dsidx-forgotpassword-confirm .dsidx-dialog-message').hide();
			$('#dsidx-passwordreset .dsidx-dialog-message').hide();
		},
		...
		Login: function (message) {
			...
			$('#dsidx-login').dialog({
				...
				position: { my: 'top', at: 'top', of: '#dsidx', collision: 'none' },
				width: 325,
				modal: true,
				resizable: false,
				draggable: false,
				dialogClass: 'dsidx-dialog dsidx-ui-widget',
				open: function () { dsidx.trackPageview('/IDX/Login'); }
			});

			if (message) {
				$('#dsidx-login .dsidx-dialog-message').show().html(message);
			}
		},

		Register: function (required) {
			this.CloseAll();
			var trackingLabel = '/IDX/Open Visitor Registration';
			if (typeof(required) != 'undefined') {
			    trackingLabel += '/Required';
			}
			$('#dsidx-register').dialog({
			    ...
			    close: function () { dsidx.trackPageview('/IDX/Abort Visitor Registration'); }
			});
		},

		Logout: function () {
		    var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=Logout';
		    if (dsidx.useWPAjax) {
		        ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=Logout';
		    }
			$.ajax({
				type: 'POST',
				dataType: 'json',
				url: ajaxUrl + '&checkExpiration=false',
				data: $('#dsidx-passwordreset form').serialize(),
				success: function (response) {
					eraseCookie('dsidx-visitor-public-id');
					eraseCookie('dsidx-visitor-auth');

					// Remove SSO Auth Cookie and redirect
					if (canUseSSO) {
						var url = getSsoSignOutPath();
						window.location = url;
						location.reload(true);
					} else {
						location.reload(true);
		...
		ForgotPasswordConfirm: function () {
			...
			$('#dsidx-forgotpassword-confirm').dialog({
...
dsidx.profile = (function ($) {
	var ProfileTabs = {};

	ProfileTabs.PersonalInfo = {
		Load: function(){
			$("#dsidx-profile-personal-loading").show();
			$("#dsidx-profile-personal-form").hide();

			var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=GetVisitor';
			if (dsidx.useWPAjax) {
			    ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=GetVisitor';
			}
			$.ajax({
				type: 'POST',
				url: ajaxUrl,
				data: { email: dsidx.visitor.Email },
                dataType: 'json',
				success: function(response) {
					if (response.Success) {
						$("#dsidx-profile-personal-info-firstname").val(response.Visitor.FirstName);
						$("#dsidx-profile-personal-info-lastname").val(response.Visitor.LastName);
						$("#dsidx-profile-personal-info-email").val(response.Visitor.Email);
						$("#dsidx-profile-personal-info-phonenumber").val(response.Visitor.PhoneNumber);
						$("#dsidx-profile-personal-info-emailupdatetype").val(response.Visitor.EmailUpdateType);
					}
				},
				complete: function(obj, status) {
					$("#dsidx-profile-personal-loading").hide();
					$("#dsidx-profile-personal-form").show();
				}
			});
		},

		Save: function(){
			$('#dsidx-profile-personal-info-button-save').html("Saving...");

			var messageDiv = $('#dsidx-profile-personal-info .dsidx-dialog-message');

			messageDiv.hide();
			messageDiv.html('');
			messageDiv.removeClass('colorred');
			messageDiv.removeClass('colorblue');

			var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=UpdatePersonalInfo';
			if (dsidx.useWPAjax) {
			    ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=UpdatePersonalInfo';
			}
			$.ajax({
				type: 'POST',
				url: ajaxUrl,
				data: $('#dsidx-profile-personal-info form').serialize(),
                dataType: 'json',
                success: function (response) {
                    // Sync visitor info
                    dsidx.visitor.EmailUpdateType = parseInt($("#dsidx-profile-personal-info-emailupdatetype").val());

					$('#dsidx-profile-personal-info-button-save').html('Save All Changes');

					// Clear the password fields
					$('#dsidx-profile-personal-info-password-1').val('');
					$('#dsidx-profile-personal-info-password-2').val('');
					
					if (response.error) {
						messageDiv.addClass("colorred");
						messageDiv.show().html(response.message);
					}
					else {
						messageDiv.addClass("colorblue");
						messageDiv.show().html("Your changes to your profile have been saved.");
					}
				}
			});
		}
	...
	ProfileTabs.Searches = {
		Load: function(){
			var panel = $('#dsidx-profile-searches');
			var list_container = document.createElement("div");

			panel.html("");
			list_container.className = "dsidx-profile-item-inner";
			panel.append(list_container);
			list_container.innerHTML = "<div class='loading'>Loading...</div>";

			var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=Searches';
			if (dsidx.useWPAjax) {
			    ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=Searches';
			}
			...
			$.ajax({
				...
				success: function(searches){
					if(searches.length == 0){
						list_container.innerHTML = "<div class='loading'>You have no saved searches.</div>";
					} else {
						var html = "Email updates are sent: <strong>"+ (dsidx.visitor.EmailUpdateType == 0 ? "never" : (dsidx.visitor.EmailUpdateType == 1 ? "nightly" : "weekly")) +"</strong>. Change this setting in your Profile.<br/><br/>";
						html += "<h2>View / Edit Your Saved Searches</h2>";

						for(var i=0,j=searches.length;i<j;i++){
							html += ProfileTabs.Searches.GenerateSearchDOM(searches[i], i);
						}
		...
		GenerateSearchDOM: function(search, index){
			var html = "<div id='dsidx-propertysearch-"+ search.PropertySearchID +"' class='dsidx-search-item"+ ((index+1) % 2 == "0" ? " dsidx-search-item-alt" : "") +"'>";
			html += "<a class='dsidx-search-delete' href='javascript:void(0)' onclick='dsidx.profile.DeleteSearch("+ search.PropertySearchID +")'>delete</a>";
			html += "<span class='dsidx-search-title' data-dsidx-property-search-id='" + search.PropertySearchID + "'><a href='" + dsidx.idxActivationPath + "?idx-q-PropertySearchID=" + search.PropertySearchID + "' data-dsidx-property-search-id='" + search.PropertySearchID + "'>" + search.Title + "</a></span>&nbsp;&nbsp;";
			html += "<input type='text' class='dsidx-search-title-input' value='' data-dsidx-property-search-id='" + search.PropertySearchID + "' />";
			html += "<a href='#' class='dsidx-search-edit-title-btn dsidx-small-button grey' data-dsidx-property-search-id='" + search.PropertySearchID + "' onclick='dsidx.profile.ToggleEditSearchTitle(this); return false;'>Edit</a>";
			html += "<a href='#' class='dsidx-search-title-save-btn dsidx-small-button grey hidden' data-dsidx-property-search-id='" + search.PropertySearchID + "' onclick='dsidx.profile.SaveEditSearchTitle(this);  return false;'>Save</a>";
			html += "<a href='#' class='dsidx-search-title-cancel-btn dsidx-small-button grey hidden' data-dsidx-property-search-id='" + search.PropertySearchID + "' onclick='dsidx.profile.ToggleEditSearchTitle(this); return false;'>Cancel</a>";

			html += "<br />";
			html += "<span class='dsidx-search-info'>" + search.ResultCount + " results, last emailed " + (typeof search.LastUpdateDate != "undefined" ? search.LastUpdateDate : "never") + "</span>";
			html += "<br />";
			html += "<span class='dsidx-search-alerts'>"+ 
						"<input id='dsidx-propertysearch-"+ search.PropertySearchID +"-alert' type='checkbox' value='yes' "+ (search.EmailSearchUpdates ? "checked='checked'" : "") +" onclick='dsidx.profile.ToggleSearchAlert("+ search.PropertySearchID +", this)' />" +
						"<label for='dsidx-propertysearch-"+ search.PropertySearchID +"-alert'>Receive New Listings Via Email</label>" +
					"</span>";
			html += "</div>";
	...
	ProfileTabs.Listings = {
		currentPage: 0,
		pageSize: 25,
		loading: false,

		Load: function(){
			var panel = $('#dsidx-profile-listings');
			panel.html("");

			var html = "<h2>Review Listings</h2>" + 
				"<ul class='dsidx-tabs dsidx-profile-listings-tabs'>" + 
					"<li class='dsidx-tab dsidx-tab-favorites'><span>Favorited</span></li>" + 
					//"<li class='dsidx-tab dsidx-tab-viewed dsidx-tab-disabled'><span>Noted</span></li>" + 
					"<li class='dsidx-tab dsidx-tab-viewed dsidx-tab-disabled'><span>Viewed</span></li>" + 
				"</ul>";
			// onclick='dsidx.profile.LoadListings(\"noted\")'
			...
			footer.innerHTML = "<a id='Listings_Print' href='javascript:void(0)' onclick='Beast.Profile.PrintListings()' class='print'>Print These Listings</a>";
			panel.append(footer);
			*/
			...
			panel.find('.dsidx-tabs .dsidx-tab').click(function(){
				var tab = $(this);

				panel.find('.dsidx-tabs .dsidx-tab').addClass('dsidx-tab-disabled');
				tab.removeClass('dsidx-tab-disabled');
		...
		LoadListings: function(page){
			...
			var panel = $('#dsidx-profile-listings');
			var list_container = $('#dsidx-profile-listings .dsidx-profile-item-inner');
			...
			list_container.append(loading);

			this.currentPage = page;

			var type = "";

			if(!$('.dsidx-tab-favorites').hasClass('dsidx-tab-disabled')) type = "favorited";
			else if(!$('.dsidx-tab-viewed').hasClass('dsidx-tab-disabled')) type = "visited";
			else if(!$('.dsidx-tab-noted').hasClass('dsidx-tab-disabled')) type = "noted";

			var thisProxy = this;
			var ajaxUrl = dsidx.pluginUrl + 'client-assist.php';
			var ajaxAction = 'VisitorListings';
			if (dsidx.useWPAjax) {
			    ajaxUrl = dsidxAjaxHandler.ajaxurl;
			    ajaxAction = 'dsidx_client_assist';
			}
			$.ajax({
				type: 'POST',
				url: ajaxUrl,
				data: { action: ajaxAction, dsidx_action: 'VisitorListings', type: type, page: page },
				success: function(response){
					thisProxy.loading = false;
					thisProxy.ResultCount = parseInt(dsidx.ExtractValueFromApiData('result_count', response));
					list_container.find('.loading').remove();
					list_container.append(response);
				}
			});
		},
		Scroll: function(list_container){
			if(
				list_container.scrollTop > 0 && 
				list_container.scrollTop + list_container.clientHeight + 300 >= list_container.scrollHeight &&
				(this.currentPage+1) * this.pageSize < this.ResultCount
			){
		...
		PrintListings: function(){
			console.log("TODO");
			/*var url = "/mvc/Visitors/Listings?favorited="+ $('Listings_Favorited').checked +"&noted="+ $('Listings_Noted').checked +"&visited="+ $('Listings_Visited').checked +"&page=0&PDF=true";
			location.href = url;*/
		}
	};
	...
	var dom_init = function(){
		$('#dsidx-profile-loggedin #dsidx-profile-personal-info-button').click(function(e){
			publicObj.Toggle('#dsidx-profile-personal-info', null, e);
		});
		$('#dsidx-profile-loggedin #dsidx-profile-searches-button').click(function(e){
			publicObj.Toggle('#dsidx-profile-searches', null, e);
		});
		$('#dsidx-profile-loggedin #dsidx-profile-listings-button').click(function(e){
			publicObj.Toggle('#dsidx-profile-listings', null, e);
		});
		$('#dsidx-profile-loggedin #dsidx-profile-logout-button').click(function(e){
			publicObj.Hide('#dsidx-profile-share', jQuery('#dsidx-profile-share'), jQuery('#dsidx-profile-sharing-button'));
			dsidx.auth.Logout(); return false;
		});

		$('#dsidx-profile-loggedout .dsidx-profile-button').click(function(){
			publicObj.Hide('#dsidx-profile-share', jQuery('#dsidx-profile-share'), jQuery('#dsidx-profile-sharing-button'));
			dsidx.auth.LoginWithSso(); return false; 
		});
		
		if ($('#dsidx-savesearch').length) {
			$('#dsidx-savesearch').dialog({
		...
		$('#dsidx-savesearch-save').click(publicObj.SaveSearch);
		$('#dsidx-savesearch-cancel').click(function(){
			$('#dsidx-savesearch').dialog('close');
		});

		publicObj.FavoriteStatus();
	};

	var GetPanel = function(dom_id){
		var panel = '';
		switch(dom_id){
			case '#dsidx-profile-personal-info': panel = 'PersonalInfo'; break;
			case '#dsidx-profile-searches': panel = 'Searches'; break;
			case '#dsidx-profile-listings': panel = 'Listings'; break;
		}
	...
	var publicObj = {
		ObserveMenu: null,
		ObserveDoc: null,
		Toggle: function(panel_id, action, e){
			...
			var panel = $(panel_id);
			var button = $(panel_id + '-button');

			if((typeof action == "undefined" || action == null) && panel.is(":visible") || action == "hide"){
				this.Hide(panel_id, panel, button);
			} else {
				this.Show(panel_id, panel, button);
			}

			return false;
		},
		Show: function(panel_id, panel, button){
			...
			ProfileTabs[GetPanel(panel_id)].Load();

			panel.show();
			button.addClass('Selected');

			dsidx.profile.ObserveMenu = function(e){
				e.stopPropagation();
			};
			dsidx.profile.ObserveDoc = function(e){
				dsidx.profile.Hide(panel_id, panel, button);
			};

			$(panel_id).click(dsidx.profile.ObserveMenu);
			$(window).click(dsidx.profile.ObserveDoc);
		},
		Hide: function(panel_id, panel, button){
			panel.hide();
			button.removeClass('Selected');

			if(typeof dsidx.profile.ObserveMenu == "function") {
				$(panel_id).unbind('click', dsidx.profile.ObserveMenu);
				dsidx.profile.ObserveMenu = null;
			}
			if(typeof dsidx.profile.ObserveDoc == "function") {
				$(window).unbind('click', dsidx.profile.ObserveDoc);
				dsidx.profile.ObserveDoc = null;
			}
		},

		Save: function(panel_id){
			ProfileTabs[GetPanel(panel_id)].Save();
		},
		
		DeleteSearch: function(id){
		    if (confirm("Are you sure you want to delete this search?")) {
		        var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=DeleteSearch';
		        if (dsidx.useWPAjax) {
		            ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=DeleteSearch';
		        }
				$.ajax({
					type: 'POST',
					url: ajaxUrl,
					data: { propertySearchID: id },
					success: function(response){
						$("#dsidx-propertysearch-"+ id).remove();
					}
				});
			}
		},

		ToggleEditSearchTitle: function(link){
		    var searchId = $(link).data('dsidx-property-search-id');
		    var searchLink = $('.dsidx-search-title[data-dsidx-property-search-id="' + searchId + '"] a');
		    var label = $(searchLink).html();
		    var searchInput = $('.dsidx-search-title-input[data-dsidx-property-search-id="' + searchId + '"]');
		    var editBtn = $('.dsidx-search-edit-title-btn[data-dsidx-property-search-id="' + searchId + '"]');
		    var saveBtn = $('.dsidx-search-title-save-btn[data-dsidx-property-search-id="' + searchId + '"]');
		    var cancelBtn = $('.dsidx-search-title-cancel-btn[data-dsidx-property-search-id="' + searchId + '"]');
		...
		SaveEditSearchTitle: function (link) {
		    var searchId = $(link).data('dsidx-property-search-id');
		    var searchLink = $('.dsidx-search-title[data-dsidx-property-search-id="' + searchId + '"] a');
		    var searchInput = $('.dsidx-search-title-input[data-dsidx-property-search-id="' + searchId + '"]');
		    var oldLabel = $(searchLink).html();
		    var newLabel = $(searchInput).val();

		    
		    if ($.trim(newLabel) != '' && newLabel != oldLabel) {
		        $(searchLink).html($(searchInput).val());
		        dsidx.profile.ToggleEditSearchTitle(link);
		        var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=UpdateSavedSearchTitle';
		        if (dsidx.useWPAjax) {
		            ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=UpdateSavedSearchTitle';
		        }
		        $.ajax({
		            type: 'POST',
		            url: ajaxUrl,
		            data: { propertySearchID: searchId, propertySearchTitle: newLabel },
		            success: function (response) {
		            }
		        });
		    }
		...
		ToggleSearchAlert: function (id, check) {
		    var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=ToggleSearchAlert';
		    if (dsidx.useWPAjax) {
		        ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=ToggleSearchAlert';
		    }
			$.ajax({
				type: 'POST',
				url: ajaxUrl,
				data: { propertySearchID: id, alert: check.checked },
                dataType: 'json',
				success: function(response){
				}
			});
		...
		LoadListings: function(){
			ProfileTabs.Listings.LoadListings(0);
		},

		FavoriteStatus: function () {
			if (!$('#dsidx-button-favorite').hasClass('dsidx-favorite-loggedin')) return;

			$('#dsidx-button-favorite').removeClass('dsidx-favorite').html('Loading...');
			
			var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=FavoriteStatus';
			if (dsidx.useWPAjax) {
			    ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=FavoriteStatus';
			}
			$.ajax({
				type: 'POST',
				url: ajaxUrl,
				data: { propertyId: dsidx.details.pid },
                dataType: 'json',
				success: function (response) {
					if (response.favorite) {
						$('#dsidx-button-favorite').addClass('dsidx-favorite');
						$('#dsidx-button-favorite').html('Remove from Favorites');
					} else {
						$('#dsidx-button-favorite').html('Save to Favorites');
					}
				}
			});
		},

		Favorite: function(propertyId){
			var favorite = $('#dsidx-button-favorite').hasClass('dsidx-favorite');

			$('#dsidx-button-favorite').html('Saving...');

			var ajaxUrl = dsidx.pluginUrl + 'client-assist.php?action=Favorite';
			if (dsidx.useWPAjax) {
			    ajaxUrl = dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=Favorite';
			}
			$.ajax({
				type: 'POST',
				url: ajaxUrl,
				data: { propertyId: propertyId, favorite: !favorite },
                dataType: 'json',
				success: function(response){
					if(favorite) {
						$('#dsidx-button-favorite').removeClass('dsidx-favorite');
						$('#dsidx-button-favorite').html('Save to Favorites');
					} else {
						$('#dsidx-button-favorite').addClass('dsidx-favorite');
						$('#dsidx-button-favorite').html('Remove from Favorites');
						if (typeof (DSListTrac) != 'undefined' && typeof (DSListTracData) != 'undefined') {
						    DSListTrac.trackEvent('favorite', DSListTracData);
						}
					}
				}
			});
		...
		SaveSearch_Start: function(){
			$('#dsidx-savesearch').dialog('open');
			$('#dsidx-savesearch-updates').attr('checked', 'checked');
			$('#dsidx-savesearch-name').val('');
		},

		SaveSearch: function(){
			$('#dsidx-savesearch-save').val('Saving...');

			$.ajax({
				type: 'GET',
				url: location.pathname,
				data: (!location.search ? "" : location.search.substring(1) + "&") + $('#dsidx-savesearch form').serialize(),
				success: function(response){
					if(!response.error){
						$('#dsidx-savesearch-save').val('Save');
						$('#dsidx-savesearch').dialog('close');
					} else {
						alert(response.message);
					}
		...
		/*PrintListings: function(){
			if(typeof this.PrintListingsDialog == "undefined"){
				var loader = new YAHOO.util.YUILoader({
					require: ['container'],
					loadOptional: false,
					onSuccess: (function () {
						var dialog = $('PrintListingsDialog');
						this.PrintListingsDialog = new YAHOO.widget.Panel(dialog, {
							modal: true,
							visible: false,
							width: "290px",
						...
						var listeners = [
							new YAHOO.util.KeyListener(
								document,
								{ keys: 27 },
								{ fn: this.PrintListingsDialog.hide, scope: this.PrintListingsDialog }
							)
						];
						this.PrintListingsDialog.cfg.queueProperty("keylisteners", listeners);
						this.PrintListingsDialog.render();
						this.PrintListingsDialog.show();

						ProfileTabs.Listings.PrintListings();
					}).bind(this)
				});
				loader.insert();
			} else {
...
var DSListTrac = {
	...
	* @pram type - Valid options: view, lead, share, gallery, favorite, vTour, clickThrough, drivingDirections, printListing
	...
	**/
    trackEvent: function (type, args) {
        ...
        if (typeof (_LT) != 'undefined' && _LT != null) {
            _LT._trackEvent(_eventType[type], args.propertyid, args.zip, args.agentid, null, null, args.ssid);
            args.type = type;
            //console.log(args);
        }
    }
